...
public class LoadingView extends View {
    ...
    private void drawUncontacArea(Canvas canvas, Ball ballA, Ball ballB) {
        ...
        //两个球径向的辅助点
        PointF pointF1 = new PointF(pointA.x - radiusA * sinValue, pointA.y + radiusA * cosValue);
        PointF pointF2 = new PointF(pointB.x - radiusB * sinValue, pointB.y + radiusB * cosValue);
        PointF pointF3 = new PointF(pointB.x + radiusB * sinValue, pointB.y - radiusB * cosValue);
        PointF pointF4 = new PointF(pointA.x + radiusA * sinValue, pointA.y - radiusA * cosValue);
        mPath.reset();
    ...
    private void drawContactArea(Canvas canvas, Ball ballA, Ball ballB) {
        ...
        //圆心距离
        int distance = calculateDistance(pointA, pointB);
        if (distance >= Math.abs(ballA.getRadius() - ballB.getRadius()) && distance < (radiusA + radiusB) * 1.5) {
            mPath.moveTo(pointF1.x, pointF1.y);
            mPath.quadTo((pointA.x + pointB.x) / 2, (pointA.y + pointB.y) / 2, pointF2.x, pointF2.y);
            mPath.lineTo(pointF3.x, pointF3.y);
            mPath.quadTo((pointA.x + pointB.x) / 2, (pointA.y + pointB.y) / 2, pointF4.x, pointF4.y);
